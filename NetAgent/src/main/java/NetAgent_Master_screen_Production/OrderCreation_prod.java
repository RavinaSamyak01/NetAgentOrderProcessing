...
public class OrderCreation_prod extends BaseInit {

	static String pck, rdytime, rectime, arrtime, tndrtime;

	public void orderCreation_production(int i) throws Exception {
		...
		try {
			...
			// -- Click on non spl link

			String Env = storage.getProperty("Env");
			if (Env.equalsIgnoreCase("PROD")) {

				try {
					WebElement non_spl = isElementPresent("non_spl_xpath");
					wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("loaderDiv")));
//			Thread.sleep(1500);
//			jse.executeScript("arguments[0].scrollIntoView(true);", non_spl); 
//			Thread.sleep(1500);
//			act.moveToElement(non_spl).build().perform();
					Thread.sleep(1500);
					act.click(non_spl).build().perform();
					logger.info("Clicked on Non SPL link");
					Thread.sleep(2000);
					// wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("loaderDiv")));

				} catch (Exception e) {
					// TODO: handle exception
					WebElement non_spl_link = isElementPresent("non_spl_xpath");
					if (!non_spl_link.isDisplayed()) {
						logger.info("Clicked on Non SPL link");
					}

					else {
						logger.info(" Non SPL link is not clicked , please Click again !!");
				...
				System.out.println(
						"This not production so not required non spl click and process normal order process as per user config");
			}
...
//				}
				...
				isElementPresent("PUAttPhone_id").clear();
				isElementPresent("PUAttPhone_id").sendKeys(PuPhone);
...
//				}
	...
	public void compareCourier(int i, String SVC)
		...
		String ExpCourierID = getData("OrderCreation", i, 38);
		...
		try {
			...
			logger.error(ee);
			logger.info("Line number is: " + ee.getStackTrace()[0].getLineNumber());
...
//			wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("loaderDiv")));
			...
			logger.info("Clicked on Search button after enter data");
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("loaderDiv")));
			try {
	...
	public void selectDropOffLoc() throws InterruptedException, IOException {
		WebDriverWait wait = new WebDriverWait(Driver, 30);
		...
		try {
			...
			logger.info(ee);
			getScreenshot(Driver, "Dropofflocationissue");
			...
			try {
				...
				FCAADD.clear();
				FCAADD.sendKeys("304 W Bay Area Blvd.");
	...
	public String shipLabel() throws IOException, InterruptedException {
		WebDriverWait wait = new WebDriverWait(Driver, 30);
		JavascriptExecutor js = (JavascriptExecutor) Driver;
		Actions act = new Actions(Driver);
		String TrackNo = null;
