...
public class AgentActivity extends BaseInit {
	...
	public void AgentActivityReport() throws Exception {
		...
		msg.append("=======Reports Tab Test Start=======" + "\n\n");
		...
		try {
			...
			getScreenshot(Driver, "AgentActivityReport2");

			logger.info("Agent Activity Report Test=PASS");
			msg.append("Agent Activity Report Test=PASS" + "\n\n");
			setResultData("Result", 22, 5, "PASS");

		} catch (Exception AgentActivityReportE) {
			logger.error(AgentActivityReportE);
			getScreenshot(Driver, "AgentActivityReport_error");
			logger.info("Agent Activity Report Test=FAIL");
			msg.append("Agent Activity Report Test=FAIL" + "\n\n");
			String Error = AgentActivityReportE.getMessage();

			setResultData("Result", 22, 5, "FAIL");
			setResultData("Result", 22, 6, Error);
